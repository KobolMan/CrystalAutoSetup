...
class UARTFlasher:
    ...
    def get_serial_from_board(self):
        """Get serial number directly from the board while in Linux mode"""
        self.logger.info("Getting serial number from board...")
        ...
        response = ""
        # Read multiple times to ensure we get the complete response
        for _ in range(5):
        ...
        if "Serial" in response:
            # Process each line to find the serial number
        ...
        self.uart.reset_input_buffer()
        self.uart.write(b"cat /sys/firmware/devicetree/base/serial-number 2>/dev/null\r\n")
        self.uart.flush()
        time.sleep(2)

        response = self.uart.read_all().decode(errors='replace')
        if response and len(response.strip()) > 8:  # Basic validation
            serial = response.strip()
            self.logger.info(f"Found serial number from devicetree: {serial}")
            return serial

        self.logger.error("Failed to get serial number from board")
        return None

    def process_mac_assignment(self, board_serial=None):
        """Complete MAC assignment process with serial check"""
        # Get serial from parameter or from the board
        serial = board_serial if board_serial else self.get_serial_from_board()
        if not serial:
            self.logger.error("Cannot proceed without serial number")
            return False

        # Check if serial already has MAC assigned
        existing_mac = self.mac_db.get_mac_for_serial(serial)
        if existing_mac:
            self.logger.info(f"Board with serial {serial} already has MAC {existing_mac}")
            # Update local boardInfo.txt in case it's different
            self.mac_db.update_board_info(serial, existing_mac)
            return True, existing_mac, "existing"

        # No existing MAC - proceed with normal flow
        mac_addr = self.mac_db.get_available_mac()
        if not mac_addr:
            self.logger.error("No available MAC addresses in database")
            return False, None, "no_available_mac"

        # Now we should be at the U-Boot prompt
        if not self.wait_for_boot_prompt():
            self.logger.error("Not at U-Boot prompt")
            return False, None, "no_uboot_prompt"

        # Write MAC address to board
        if not self.write_mac_address(mac_addr):
            self.logger.error("Failed to write MAC address")
            return False, None, "write_failed"

        # Update database
        if not self.mac_db.mark_mac_as_used(mac_addr, serial):
            self.logger.error("Failed to update database")
            return False, mac_addr, "database_update_failed"

        self.logger.info(f"Successfully assigned MAC {mac_addr} to board {serial}")
        return True, mac_addr, "success"
